...
st.sidebar.write('■　1_JPON出力データの読み込み')
...
uploaded_file = st.sidebar.file_uploader("3_佐川急便（承天貿易）運賃ファイルを読み込んでください", type="csv")
if uploaded_file is not None:
...
uploaded_file = st.sidebar.file_uploader("4_佐川急便（GOODLIFE）運賃ファイルを読み込んでください", type="csv")
if uploaded_file is not None:
...
uploaded_file = st.sidebar.file_uploader("5_佐川急便（昌隆）運賃ファイルを読み込んでください", type="csv")
if uploaded_file is not None:
...
uploaded_file = st.sidebar.file_uploader("6_西濃運輸 運賃ファイルを読み込んでください", type="csv")
if uploaded_file is not None:
...
uploaded_file = st.sidebar.file_uploader("7_セイノーSSX 運賃ファイルを読み込んでください", type="csv")
if uploaded_file is not None:
...
uploaded_file = st.sidebar.file_uploader("8_福山通運 運賃ファイルを読み込んでください", type="csv")
if uploaded_file is not None:
...
#直接引き取りに０を入力
main_data_df_single_item_ship.loc[main_data_df_single_item_ship['配送業者'] == '直接引取', '直接引取０円'] = 0
...
[['受注番号', '顧客分類_x', '佐川急便運賃_x', '西濃運輸運賃', 'ssx運賃', '福山通運運賃', 'ゆうパック運賃','クリックポスト運賃','定形外送料']]
...
#直接引き取りに０を入力
main_data_df_set_item_ship.loc[main_data_df_set_item_ship['配送業者'] == '直接引取', '直接引取０円'] = 0
...
main_data_df_set_item_ship = main_data_df_set_item_ship[['受注番号', '顧客分類_x', '佐川急便運賃_x', '西濃運輸運賃', 'ssx運賃',\
...
item_price_total_ship['運賃'] = item_price_total_ship['佐川急便運賃_x'] + item_price_total_ship['西濃運輸運賃'] + item_price_total_ship['ssx運賃']\
...
st.write('※日通トランスポート、九州航空は含まないので別途計算します。')
